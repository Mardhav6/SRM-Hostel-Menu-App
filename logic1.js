...
let weekNumber = getWeekNumber(now); // Get the week number
...
// Function to display the next meal
function nextMeal() {
    ...
    if (hour >= 0 && hour < 9) {
        mealIndex = 1; // Next meal is Lunch
    } else if (hour >= 9 && hour < 14) {
        mealIndex = 2; // Next meal is Snacks
    } else if (hour >= 14 && hour < 17) {
        mealIndex = 3; // Next meal is Dinner
    } else {
        // It's already past dinner, so go to the next day's Breakfast
    ...
    displayMenu(menu, mealIndex);
}

// Function to display the previous meal
function previousMeal() {
    ...
    if (hour >= 0 && hour < 9) {
        // It's currently Breakfast, so go to the previous day's Dinner
        dayOfWeek = (dayOfWeek + 6) % 7;
        mealIndex = 3;
    } else if (hour >= 9 && hour < 14) {
        mealIndex = 0; // Previous meal is Breakfast
    } else if (hour >= 14 && hour < 17) {
        mealIndex = 1; // Previous meal is Lunch
    } else {
        mealIndex = 2; // Previous meal is Snacks
    }
...
// Function to clear the displayed menu
function clearMenu() {
    mealInfo1.textContent = '';
    mealInfo2.textContent = '';
    mealInfo3.textContent = '';
    mealInfo4.textContent = '';
}
